...
export default class TestFairy {
  ...
  constructor(params) {
    ...
    this.icon_watermark  = params.icon_watermark && params.icon_watermark == "off" ? "off" : "on";
  ...
  get_common_params() {
    let params = [
      ...
      '-F', 'testers-groups='+this.groups,
      '-F', 'notify='+this.notify,
      '-F', 'auto-update='+this.auto_update,
      '-F', 'video='+this.video,
      '-F', 'max-duration='+this.max_duration,
      '-F', 'icon-watermark='+this.icon_watermark
    ];
    if(this.comment != undefined && this.comment != "") {
      params.push('-F')
      params.push('comment='+this.comment)
    }
    return params;
  }
